    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard icon={<CurrencyDollarIcon className="h-6 w-6 text-brand-blue-dark" />} title="Total Revenue" value={`$${stats.totalRevenue.toFixed(2)}`} />
                <StatCard icon={<PackageIcon className="h-6 w-6 text-brand-blue-dark" />} title="Total Orders" value={stats.totalOrders.toString()} />
                <StatCard icon={<UsersIcon className="h-6 w-6 text-brand-blue-dark" />} title="New Customers" value={stats.newCustomers.toString()} />
                    ...
                    <div style={{ width: '100%', height: 300 }}>
                        <ResponsiveContainer>
                            <BarChart data={weeklySales}>
                                ...
                                <Bar dataKey="sales" fill="#3182CE" name="Sales ($)" barSize={30} radius={[4, 4, 0, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </Card>
                        ...
                        {recentOrders.map(order => (
                            <div key={order.id} className="flex justify-between items-center">
                                ...
                                <div className="text-right">
                                     <p className="font-semibold text-brand-gray-800">${order.total.toFixed(2)}</p>
